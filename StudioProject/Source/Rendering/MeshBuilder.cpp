...
Mesh* MeshBuilder::GenerateBar(const std::string &meshName, Color color,float barValue, float length)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	//v.texCoord.Set(0, 0);
	//vertex_buffer_data.push_back(v);
	//v.pos.Set(0.5f * barValue, -0.5f * length, 0);
	//v.color = color;
	//v.normal.Set(0, 0, 1);
	//v.texCoord.Set(1.0f, 0);
	//vertex_buffer_data.push_back(v);
	//v.pos.Set(0.5f + 0.5f*barValue, 0.5f * length, 0);
	//v.color = color;
	//v.normal.Set(0, 0, 1);
	//v.texCoord.Set(1.0f, 1.0f);
	//vertex_buffer_data.push_back(v);
	////v.pos.Set(0.25 * length, 0.5f * length, 0);
	//v.pos.Set(-0.5 * length + 0.5f * barValue, 0.5f * length, 0);
	//v.color = color;
	//v.normal.Set(0, 0, 1);
	//v.texCoord.Set(0, 1.0f);
	//vertex_buffer_data.push_back(v);

	v.pos.Set(-length/2, -length / 2, 0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(0, 0);
	vertex_buffer_data.push_back(v);
	...
	v.texCoord.Set(1.0f, 0);
	vertex_buffer_data.push_back(v);
	v.pos.Set(length/2 + 1, length/2, 0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(1.0f, 1.0f);
	vertex_buffer_data.push_back(v);
...
Mesh* MeshBuilder::GenerateCircle(const std::string &meshName, Color color, float length)
{
	...
	for (int i = 0; i <= 36; ++i)
	{
		yPos = sin(Math::DegreeToRadian(i * 10.0f)) * 0.5f * length;
		xPos = cos(Math::DegreeToRadian(i * 10.0f)) * 0.5f * length;
